%% PRE-PROCESSING OF DATA IN HIGH FREQUENCY BAND FOR R1

function process_rx_hf_r1()
% ========================================================================
%  MAIN SCRIPT 
% ========================================================================
...
fpath = fullfile(fpath, 'SFI Smart Ocean Dataset', 'RX-HF', 'R1');
...
modulation_types = {
    ...
    'A-PRBS',     60.011000000000003,   0, 90000;
    'OFDM',       7.468958333333333,    0, 96000;
    'OSDM',       3.890750000000000,    0, 96000;
    'MFSK',       4.000000000000000,    0, 96000;
    'JANUS',      0.704000000000000,    0, 96000;
    'SC-QPSK',    1.183166666666667,    0, 96000;
};
...
cycle_numbers = [2, 5, 8, 11, 13, 16, 18, 19, 20, 23, 24, 25, 32, 35, 39, 40, 42, 53, 61, 71, 77, 79, 80];
for idx = 1:length(cycle_numbers)
    % Load the rx_data
    cycle_num = cycle_numbers(idx)
    filename = sprintf('HF_R1_cycle%02d.wav', cycle_num);
    [rx_data, fs_rx] = audioread(fullfile(fpath, filename));
    rx_data = rx_data/ max(abs(rx_data));

    % Low energy detection 
    low_energy_segments = detect_low_energy(rx_data, fs_rx, 0.09, 0.8);
