...
rx_path = fullfile(script_folder, 'RX', 'R1','MULTITONE');
tx_path = fullfile(script_folder, 'TX');

% Load Tx data 
tx_struct = load(fullfile(tx_path, 'MULTITONE.mat'));
tx_data = tx_struct.tx_data_final;

% Modulation scheme
modulation_scheme = 'MULTITONE';
...
title_str = sprintf('Adaptive ARMAX Model for R1 - Real Part [%s] (Fit: %.2f%%)', ...
...
title_str = sprintf('Adaptive ARMAX Model for R1 - Imaginary Part [%s] (Fit: %.2f%%)', ...
